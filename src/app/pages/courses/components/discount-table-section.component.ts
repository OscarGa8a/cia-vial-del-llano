...
@Component({
  ...
  template: `
    <section class="py-20 bg-white" aria-labelledby="discount-table-heading">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <!-- Section header -->
        <div class="text-center mb-14">
          <span
            class="inline-block bg-accent-light text-accent text-xs font-bold uppercase tracking-widest px-4 py-1.5 rounded-full mb-4"
          >
            Plazos de descuento
          </span>
          <h2
          ...
          <p class="text-text-secondary text-lg max-w-2xl mx-auto">
            El porcentaje de descuento depende del tipo de comparendo y del momento en que realices
            el curso. ¡Actúa rápido!
          </p>
        </div>
              ...
              <div class="bg-white p-8">
                <div class="flex flex-col gap-4 mb-8">
                  @for (tier of card.tiers; track tier.percent + tier.label) {
                    <div
                      class="flex items-center justify-between p-4 rounded-2xl bg-gray-50 border border-border"
                    >
                      <div>
        ...
        >
          ...
          <div>
            ...
            <p class="text-sm text-text-secondary mt-1">
              Los días hábiles son de lunes a viernes, sin contar festivos. El conteo comienza desde
              el día siguiente a la fecha de notificación del comparendo.
              <strong>No esperes al último momento</strong> para tomar tu curso.
            </p>
          </div>
        </div>
      </div>
...
export class DiscountTableSectionComponent {
  ...
  protected getWhatsappLink(message: string): string {
    return this.whatsapp.generateLink(message);
  }
}

import { ChangeDetectionStrategy, Component, inject } from '@angular/core';
import { DomSanitizer } from '@angular/platform-browser';
import { CONFIG } from '../../../core/data/config.data';

/**
 * Location preview section showing the company address, hours,
 * contact info, navigation buttons and embedded Google Map.
 */
@Component({
  selector: 'app-location-section',
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <section
      class="py-20 bg-white"
      id="ubicacion"
      aria-labelledby="location-heading"
    >
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <!-- Section header -->
        <div class="text-center mb-14">
          <span class="inline-block bg-primary-lighter text-primary text-xs font-bold uppercase tracking-widest px-4 py-1.5 rounded-full mb-4">
            Ubicación
          </span>
          <h2
            id="location-heading"
            class="font-sans font-bold text-3xl sm:text-4xl text-text-primary mb-4"
          >
            Visítanos en nuestra sede
          </h2>
          <p class="text-text-secondary text-lg max-w-2xl mx-auto">
            Estamos ubicados en Villavicencio, Meta. Fácil acceso y estacionamiento disponible.
          </p>
        </div>

        <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
          <!-- Info column -->
          <div class="flex flex-col gap-6">
            <!-- Address card -->
            <div class="bg-surface-alt rounded-2xl p-6 border border-border">
              <h3 class="font-sans font-bold text-lg text-text-primary mb-4 flex items-center gap-2">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="text-primary" aria-hidden="true">
                  <path d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/>
                </svg>
                Dirección
              </h3>
              <p class="text-text-secondary text-sm leading-relaxed">{{ config.address.full }}</p>
            </div>

            <!-- Hours card -->
            <div class="bg-surface-alt rounded-2xl p-6 border border-border">
              <h3 class="font-sans font-bold text-lg text-text-primary mb-4 flex items-center gap-2">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="text-primary" aria-hidden="true">
                  <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
                </svg>
                Horarios de atención
              </h3>
              <div class="flex flex-col gap-2">
                @for (schedule of schedules; track schedule.days) {
                  <div class="flex items-center justify-between text-sm">
                    <span class="text-text-secondary">{{ schedule.days }}</span>
                    <span
                      class="font-medium"
                      [class.text-text-primary]="schedule.open"
                      [class.text-error]="!schedule.open"
                    >{{ schedule.hours }}</span>
                  </div>
                }
              </div>
            </div>

            <!-- Contact card -->
            <div class="bg-surface-alt rounded-2xl p-6 border border-border">
              <h3 class="font-sans font-bold text-lg text-text-primary mb-4 flex items-center gap-2">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="text-primary" aria-hidden="true">
                  <path d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
                </svg>
                Contáctanos
              </h3>
              <div class="flex flex-col gap-3">
                <a
                  [href]="'tel:' + config.contact.phone.replace(' ', '')"
                  class="flex items-center gap-2 text-sm text-primary hover:underline"
                  aria-label="Llamar al teléfono"
                >
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
                    <path d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
                  </svg>
                  {{ config.contact.phone }}
                </a>
                <a
                  [href]="config.social.whatsapp"
                  target="_blank"
                  rel="noopener noreferrer"
                  class="flex items-center gap-2 text-sm text-whatsapp hover:underline"
                  aria-label="Abrir WhatsApp"
                >
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                  </svg>
                  Escribir por WhatsApp
                </a>
              </div>
            </div>

            <!-- Navigation buttons -->
            <div class="grid grid-cols-2 gap-3">
              <a
                [href]="config.googleMapsUrl"
                target="_blank"
                rel="noopener noreferrer"
                class="flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-primary hover:bg-primary-light text-white font-semibold text-sm transition-all"
                aria-label="Abrir en Google Maps"
              >
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
                  <path d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/>
                </svg>
                Google Maps
              </a>
              <a
                [href]="config.wazeUrl"
                target="_blank"
                rel="noopener noreferrer"
                class="flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 border-primary text-primary hover:bg-primary hover:text-white font-semibold text-sm transition-all"
                aria-label="Abrir en Waze"
              >
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
                  <circle cx="12" cy="12" r="10"/><circle cx="12" cy="10" r="3"/><path d="M7 20.662V19a2 2 0 012-2h6a2 2 0 012 2v1.662"/>
                </svg>
                Waze
              </a>
            </div>
          </div>

          <!-- Map column -->
          <div class="rounded-3xl overflow-hidden shadow-xl border border-border h-[500px] lg:h-full min-h-[400px]">
            <iframe
              [src]="mapSrc"
              width="100%"
              height="100%"
              style="border:0;display:block;"
              allowfullscreen
              loading="lazy"
              referrerpolicy="no-referrer-when-downgrade"
              title="Ubicación de CIA Vial del Llano en Villavicencio, Meta"
            ></iframe>
          </div>
        </div>
      </div>
    </section>
  `,
})
export class LocationSectionComponent {
  private readonly sanitizer = inject(DomSanitizer);
  protected readonly config = CONFIG;

  /** SafeResourceUrl: bypasses Angular's iframe src sanitization for this trusted Google Maps embed URL. */
  protected readonly mapSrc = this.sanitizer.bypassSecurityTrustResourceUrl(CONFIG.mapEmbedUrl);

  protected readonly schedules = [
    { days: 'Lunes a Viernes', hours: '8:00 AM – 6:00 PM', open: true },
    { days: 'Sábados', hours: '8:00 AM – 2:00 PM', open: true },
    { days: 'Domingos', hours: 'Cerrado', open: false },
  ] as const;
}
